...
class CityBusDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    def add_route_stop(self, route_name: str, direction_destination: str, stop_code: str) -> None:
    ...
    def remove_route_stop(self, route_name: str, direction_destination: str, stop_code: str) -> None:
        """Tell coordinator to stop tracking a given stop for a route and direction."""
        self._route_stops.remove(RouteStop(route_name, direction_destination, stop_code))

    def get_estimate_data(self, route_name: str, direction_destination: str, stop_code: str) -> dict[str, Any] | None:
        """Get the estimate data for a given stop for a route and direction."""
        return self._estimates.get(RouteStop(route_name, direction_destination, stop_code))
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        def _update_data() -> dict:
            ...
            for route_stop in self._route_stops:
                try:
                    route_key = self.citybussin.get_route_by_short_name(route_stop.route_name)["key"]
                    direction_key = self.citybussin.get_direction_by_destination(route_key, route_stop.direction_destination)["direction"]["key"]
